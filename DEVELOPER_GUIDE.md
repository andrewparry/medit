...
## URL Hash Parameter Support

### Implementation Overview

The editor supports opening files via URL hash parameters, allowing direct file loading on page initialization.

### Hash Parsing

**Location**: `js/editor-init.js`
...
- `#/absolute/path/to/file.md` - Absolute path (relative to origin)
- `#http://example.com/file.md` - HTTP URL
- `#https://example.com/file.md` - HTTPS URL
- `#file:///path/to/file.md` - File protocol (shows error, cannot load directly)

### File Loading from Path

**Location**: `js/editor-file-ops.js`
...
- Network errors: Shows user-friendly error dialog
- File not found (404): Shows error message
- File too large (>10MB): Validates and rejects
- Invalid path format: Shows format error
...
const initialize = async () => {
    ...
    // Try to load file from URL hash before restoring autosave
    const hashFileLoaded = await loadFileFromHash();

    if (MarkdownEditor.autosave) {
        MarkdownEditor.autosave.checkAutosaveStatus();
        // Only restore autosave if no file was loaded from hash
        if (!hashFileLoaded) {
...
1. Check URL hash for file path
2. If hash exists, attempt to load file
3. If hash file fails, show error and start with empty editor
4. If no hash, restore autosave (existing behavior)

### URL Hash Updates

When files are opened via UI, the URL hash is automatically updated:
...
## Multi-Tab Editing Architecture

### Tab State Isolation

**Location**: `js/editor-state.js`

Each browser tab maintains isolated state using unique tab IDs:
...
- Tab ID persisted in `sessionStorage` (cleared when tab closes)
- Each tab gets unique identifier on first load
- Tab ID reused if page reloads in same tab

### Tab-Specific Autosave

**Location**: `js/editor-autosave.js`

Autosave keys are prefixed with tab ID to prevent conflicts:
...
- `markdown-editor-autosave-{tabId}` - Editor content
- `markdown-editor-filename-{tabId}` - Filename

**Benefits**:

- Each tab has independent autosave
- No cross-tab interference
- Tab-specific restoration on reload
...
- Programmatic: `MarkdownEditor.fileOps.openFileInNewTab('/path/to/file.md')`
- Manual: Open editor with different URL hashes in separate tabs

### State Isolation Strategy

**Per-Tab Isolated**:

- Editor content (`elements.editor.value`)
- Autosave data (tab-specific localStorage keys)
- File handles (`currentFileHandle`, `currentDirectoryHandle`)
- Editor state (`state.dirty`, `state.lastSavedContent`)

**Shared Across Tabs**:

- Theme preference (`localStorage` without tab prefix)
- HTML rendering preference
- Split ratio preference

**Implementation Notes**:

- Each browser tab = separate JavaScript execution context
- No cross-tab communication needed
- `sessionStorage` ensures tab-specific persistence
- `localStorage` with tab prefix ensures isolated autosave
