...
class FileManager {
    ...
    setupEditableFileName() {
        ...
        const finishEditing = (save = true) => {
            ...
            if (save && newFileName && this.isValidFileName(newFileName)) {
                ...
                // Announce error
                if (this.editorCore.accessibilityManager) {
                    this.editorCore.accessibilityManager.announce(
                        'Invalid file name. Reverted to original name.'
                    );
                }
                return;
            } else {
    ...
    updateFileNameDisplay(fileName) {
        ...
        if (fileNameElement) {
            ...
            fileNameElement.classList.toggle(
                'unsaved',
                this.editorCore.state.file.hasUnsavedChanges
            );

            // Update title attribute for accessibility
            fileNameElement.setAttribute(
                'title',
                `Click to edit file name: ${fileName || 'Untitled'}`
            );
            fileNameElement.setAttribute(
                'aria-label',
                `File name: ${fileName || 'Untitled'}. Click to edit.`
            );
        }
    }
}
